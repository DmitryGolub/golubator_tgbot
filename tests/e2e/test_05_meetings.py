...
async def test_create_meeting_fsm(
    ...
    student_btn = _find_button(create_msg, "meeting_student:")
    ...
    if skip_type_btn:
        ...
        type_btn = _find_button(type_msg, "meeting_type:")
    ...
    date_btn = _find_button(date_msg, "meeting_date:")
    assert date_btn is not None, (
        f"Should find date button in calendar. Buttons: "
        f"{[(b.text, b.data.decode()) for b in _get_buttons(date_msg)]}"
    )
    ...
    skip_link_btn = _find_button(link_msg, "meeting_skip_link")
    if skip_link_btn:
...
async def test_start_call(
    ...
    assert start_btn is not None, (
        ...
        f"{[(b.text, b.data.decode()) for b in _get_buttons(meetings_msg)]}"
    )
